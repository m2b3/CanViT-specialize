...
from canvit_specialize.datasets.in1k import (
    NUM_CLASSES,
    load_class_names,
    load_real_labels,
    real_accuracy,
)
from canvit_specialize.training.in1k.config import TrainConfig
from canvit_specialize.training.in1k.eval import evaluate
from canvit_specialize.training.in1k.extract import CLS_SUFFIX, FILENAMES_SUFFIX, LABELS_SUFFIX
from canvit_specialize.training.in1k.loss import get_loss_fn
from canvit_specialize.training.in1k.naming import features_split_dir, model_name_from_repo
...
def load_train_features(cfg: TrainConfig) -> tuple[list[Tensor], list[Tensor], int]:
    ...
    d = features_split_dir(cfg.features_dir, model_repo=cfg.model_repo, image_size=cfg.image_size, split="train")
...
def load_val_features(cfg: TrainConfig, embed_dim: int) -> tuple[Tensor, Tensor, list[str]]:
    d = features_split_dir(cfg.features_dir, model_repo=cfg.model_repo, image_size=cfg.image_size, split="val")
...
def _sample_optimizer(
    ...
    """Sample optimizer type + HPs. Returns (optimizer, hp_dict)."""
