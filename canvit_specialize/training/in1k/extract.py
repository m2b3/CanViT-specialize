...
Two-phase pipeline: extract features once, train probes many times.
This module handles phase 1: running the frozen DINOv3 backbone over
ImageNet train/val splits and caching CLS tokens to disk.

Features are stored as .pt files under
...
from canvit_specialize.datasets.in1k_dali import create_loader
from canvit_specialize.training.in1k.config import ExtractionConfig
from canvit_specialize.training.in1k.naming import features_split_dir
...
def _output_prefix(
    features_dir: Path, *, split: str, model_repo: str, image_size: int,
) -> Path:
    split_dir = features_split_dir(features_dir, model_repo=model_repo, image_size=image_size, split=split)
    split_dir.mkdir(parents=True, exist_ok=True)
    if split == "train":
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        uid = str(uuid.uuid4())[:8]
        return split_dir / f"{ts}_{uid}"
    return split_dir / "features"


def _extract_cls(teacher: DINOv3Teacher, images: Tensor) -> Tensor:
    with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
        feats = teacher.forward_norm_features(images)
    return feats.cls.float()
...
def run_extraction(cfg: ExtractionConfig) -> None:
    ...
    log.info("Loading teacher: %s", cfg.model_repo)
    teacher = load_teacher(repo_id=cfg.model_repo, device=device)
    ...
    prefix = _output_prefix(
        cfg.features_dir, split=cfg.split, model_repo=cfg.model_repo, image_size=cfg.image_size,
    )
    wip_files: list[Path] = []
    ...
    with tqdm(total=total_images, desc=cfg.split) as pbar:
        for batch_idx, batch in enumerate(it):
            ...
            cls_token = _extract_cls(teacher, images)
