"""NVIDIA DALI GPU data loader for ImageNet-1K.

Linux + CUDA only. Imports are deferred so this module can be imported
(but not called) on macOS for type-checking.
"""

import logging
import math
from collections.abc import Iterator
from dataclasses import dataclass

from timm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD

log = logging.getLogger(__name__)

# DALI normalizes uint8 images, so mean/std are in [0, 255] range.
_MEAN_U8 = [m * 255 for m in IMAGENET_DEFAULT_MEAN]
_STD_U8 = [s * 255 for s in IMAGENET_DEFAULT_STD]


@dataclass
class DALILoader:
    """Thin wrapper around a DALI pipeline iterator."""

    _iterator: Iterator
    images_per_epoch: int
    batch_size: int

    def __iter__(self) -> Iterator:
        return self._iterator


def create_loader(
    data_root: str,
    batch_size: int,
    image_size: int,
    *,
    training: bool = False,
    return_filenames: bool = False,
    num_threads: int = 16,
    device_id: int = 0,
) -> DALILoader:
    """Build a DALI pipeline that yields normalized (B,3,H,W) CUDA tensors.

    DALI always returns full batches (wraparound). Callers must slice the
    final batch if epoch_size % batch_size != 0.
    """
    # Defer heavy GPU-only imports (DALI, cupy).
    import cupy as cp  # type: ignore[import-untyped]
    import nvidia.dali.fn as fn  # type: ignore[import-untyped]
    import torch
    from nvidia.dali import pipeline_def  # type: ignore[import-untyped]
    from nvidia.dali.pipeline import DataNode  # type: ignore[import-untyped]
    from nvidia.dali.types import DALIDataType, DALIImageType  # type: ignore[import-untyped]

    reader_name = "TrainReader" if training else "ValReader"
    shuffle = training

    log.info("DALI pipeline: %s %dpx BS=%d shuffle=%s threads=%d",
             data_root, image_size, batch_size, shuffle, num_threads)

    @pipeline_def()  # type: ignore[misc]
    def pipe():
        jpegs, labels = fn.readers.file(
            file_root=data_root, random_shuffle=shuffle,
            name=reader_name, pad_last_batch=True,
        )
        images = fn.decoders.image(jpegs, device="mixed", output_type=DALIImageType.RGB)

        if training:
            images = fn.random_resized_crop(images, size=(image_size, image_size))
            flip = fn.random.coin_flip(probability=0.5)
        else:
            images = fn.resize(images, mode="not_smaller", size=(image_size, image_size))
            flip = 0

        assert isinstance(images, DataNode)
        images = fn.crop_mirror_normalize(
            images, mirror=flip, crop=(image_size, image_size),
            dtype=DALIDataType.FLOAT, output_layout="CHW",
            mean=_MEAN_U8, std=_STD_U8,
        )
        return (images, labels, jpegs) if return_filenames else (images, labels)

    p = pipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id)  # type: ignore[operator]
    p.build()

    epoch_size = p.epoch_size()[reader_name]
    n_batches = math.ceil(epoch_size / batch_size)
    log.info("  epoch_size=%d  n_batches=%d", epoch_size, n_batches)

    def iterator() -> Iterator[dict]:
        for _ in range(n_batches):
            outputs = p.run()
            if return_filenames:
                img_dali, lbl_dali, jpeg_dali = outputs
                fnames = [jpeg_dali[i].source_info() for i in range(len(jpeg_dali))]
            else:
                img_dali, lbl_dali = outputs
                fnames = None

            images = torch.as_tensor(cp.asarray(img_dali.as_tensor()), device="cuda")
            labels = torch.as_tensor(cp.asarray(lbl_dali.as_tensor()), device="cuda")
            torch.cuda.synchronize()

            batch: dict = {"images": images, "labels": labels}
            if fnames is not None:
                batch["filenames"] = fnames
            yield batch

    return DALILoader(_iterator=iterator(), images_per_epoch=epoch_size, batch_size=batch_size)
